    ...
    <div className="w-full h-15 flex fixed top-0 bg-background justify-center items-center border-b border-line">
      <div className="w-[800px] border-r border-line h-full flex justify-between items-center px-1">
        ...
        <div className="md:w-[70%] w-[50%] h-full gap-2 flex justify-end items-center">
          {/* NavLinks */}
          <div className="md:flex hidden gap-4 justify-center border-r border-line pr-4">
            {links.map((item, idx) => (
              <Link
                className="text-[14px] font-medium text-secondary-text hover:text-primary-text transition-all duration-200 ease-in-out border--[1.5px] dark:border-black border-white hover:border-primary-text relative"
                href={item.href}
                key={idx}
              >
                {item.name}
...
const GithubSVG = () => {
  return (
    <svg
      ...
      className="fill-secondary-text group-hover:fill-primary-text transition-all duration-300 ease-in-out"
    >
